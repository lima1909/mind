...
func (mi *idMapIndex[OBJ, ID]) Equal(value any) (*lidx.RawIDs32, error) {
	id, ok := value.(ID)
	if !ok {
		return nil, errr.InvalidValueTypeError[ID]{Value: value}
	}

	idx, found := mi.GetIndex(id)
	if !found {
		return nil, errr.ValueNotFoundError{Value: id}
	}
...
func (mi *idMapIndex[OBJ, ID]) Match(_ *lidx.RawIDs32, op query.FilterOp, _ any) (*lidx.RawIDs32, bool, error) {
	return nil, false, errr.InvalidOperationError{IndexName: IDMapIndexName, Op: op.Op}
}
...
func (*idSliceIndex[OBJ, ID]) Match(_ *lidx.RawIDs32, op query.FilterOp, _ any) (*lidx.RawIDs32, bool, error) {
	return nil, false, errr.InvalidOperationError{IndexName: IDSliceIndexName, Op: op.Op}
}
