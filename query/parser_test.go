...
func TestParser_Base(t *testing.T) {
	...
	_ = indexMap.AddIndex("id", index.NewMapIndex((*User).ID))
	_ = indexMap.AddIndex("name", index.NewStringIndex((*User).Name).AddPhoneticIndex().AddFuzzyIndex().AddTrigramIndex())
	...
	indexMap.Insert(&User{id: 40, name: "Paul\\'s", role: "developer", price: 3.0, ok: true}, 0)
	indexMap.Insert(&User{id: 42, name: "Alice", role: "admin", price: 1.2, ok: false}, 1)
...
func TestParser_Cast(t *testing.T) {
	...
	indexMap := mind.NewIndexMap[data]()

	_ = indexMap.AddIndex("u", index.NewSortedIndex(index.FromName[data, uint]("U")))
	_ = indexMap.AddIndex("u8", index.NewSortedIndex(index.FromName[data, uint8]("U8")))
	_ = indexMap.AddIndex("u16", index.NewSortedIndex(index.FromName[data, uint16]("U16")))
	_ = indexMap.AddIndex("u32", index.NewSortedIndex(index.FromName[data, uint32]("U32")))
	_ = indexMap.AddIndex("u64", index.NewSortedIndex(index.FromName[data, uint64]("U64")))
	indexMap.Insert(&data{U: 42}, 1)
	indexMap.Insert(&data{U8: 5}, 2)
	indexMap.Insert(&data{U16: 16}, 3)
	indexMap.Insert(&data{U32: 32}, 4)
	indexMap.Insert(&data{U64: 64}, 5)

	_ = indexMap.AddIndex("i", index.NewSortedIndex(index.FromName[data, int]("I")))
	_ = indexMap.AddIndex("i8", index.NewSortedIndex(index.FromName[data, int8]("I8")))
	_ = indexMap.AddIndex("i16", index.NewSortedIndex(index.FromName[data, int16]("I16")))
	_ = indexMap.AddIndex("i32", index.NewSortedIndex(index.FromName[data, int32]("I32")))
	_ = indexMap.AddIndex("i64", index.NewSortedIndex(index.FromName[data, int64]("I64")))
	indexMap.Insert(&data{I: -42}, 6)
	indexMap.Insert(&data{I8: -8}, 7)
	indexMap.Insert(&data{I16: -16}, 8)
	indexMap.Insert(&data{I32: -32}, 9)
	indexMap.Insert(&data{I64: -64}, 10)

	_ = indexMap.AddIndex("f32", index.NewSortedIndex(index.FromName[data, float32]("F32")))
	_ = indexMap.AddIndex("f64", index.NewSortedIndex(index.FromName[data, float64]("F64")))
	indexMap.Insert(&data{F32: -3.2}, 11)
	indexMap.Insert(&data{F64: -6.4}, 12)
	...
	tests := []struct {
		...
		{query: `u8  = 5`, expected: []uint32{2}},
		{query: `u16 = 16`, expected: []uint32{3}},
		{query: `u32 = 32`, expected: []uint32{4}},
		{query: `u64 = 64`, expected: []uint32{5}},

		{query: `i   = -42`, expected: []uint32{6}},
		{query: `i8  = -8`, expected: []uint32{7}},
		{query: `i16 = -16`, expected: []uint32{8}},
		{query: `i32 = -32`, expected: []uint32{9}},
		{query: `i64 = -64`, expected: []uint32{10}},

		{query: `f32 = -3.2`, expected: []uint32{11}},
		{query: `f64 = -6.4`, expected: []uint32{12}},
	}
...
func TestParser_ImpossibleRange(t *testing.T) {
	...
	indexMap.Insert(&User{role: "developer", price: 3.0, ok: true}, 0)
	indexMap.Insert(&User{role: "admin", price: 1.2, ok: false}, 1)
...
func TestParser_UDF(t *testing.T) {
	...
	_ = indexMap.AddIndex("name", newUdfIndex((*User).Name))
	_ = indexMap.AddIndex("price", newUdfIndex((*User).Price))

	indexMap.Insert(&User{name: "", price: 3.0}, 0)
	indexMap.Insert(&User{name: "Alice", price: 1.2}, 1)
...
type udfIndex[OBJ any, V comparable, LI lidx.UInt] struct {
	...
	fieldGetFn index.FromField[OBJ, V]
}

func newUdfIndex[OBJ any, V comparable](fromField index.FromField[OBJ, V]) index.Index[OBJ] {
...
func (mi *udfIndex[OBJ, V, LI]) Equal(value any) (*lidx.RawIDs[LI], error) {
	v, err := index.ValueFromAny[V](value)
	if err != nil {
		return nil, errr.InvalidValueTypeError[V]{Value: value}
	}
...
func (mi *udfIndex[OBJ, V, LI]) Match(_ *lidx.RawIDs32, op query.FilterOp, value any) (*lidx.RawIDs[LI], bool, error) {
	if op != udfOp {
		return nil, false, errr.InvalidOperationError{IndexName: index.MapIndexName, Op: op.Op}
	}
...
func (mi *udfIndex[OBJ, V, LI]) MatchMany(op query.FilterOp, values ...any) (*lidx.RawIDs[LI], bool, error) {
	...
	case udfOp, query.FOpIn:
		...
		for _, v := range values {
			key, err := index.ValueFromAny[V](v)
			if err != nil {
