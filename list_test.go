...
func TestList_ListFilter(t *testing.T) {
	...
	result, err = il.QueryStr(`age > 21`).Values()
	assert.NoError(t, err)
	assert.Equal(t, []car{
		{name: "Opel", age: 22},
		{name: "Dacia", age: 22},
	...
	result, err = il.QueryStr(`age < 6`).Values()
	assert.NoError(t, err)
	assert.Equal(t, []car{{name: "Mercedes", age: 5}}, result)
	...
	result, err = il.QueryStr(`age between(6, 22)`).Values()
	assert.NoError(t, err)
	assert.Equal(t, []car{
		{name: "Opel", age: 22},
		{name: "Dacia", age: 22},
	...
	result, err = il.QueryStr(`age IN(6, 22)`).Values()
	assert.NoError(t, err)
	assert.Equal(t, []car{
		{name: "Opel", age: 22},
		{name: "Dacia", age: 22},
