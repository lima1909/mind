...
// ValuesBetween iterates over set by starting from 'skipN' until the end or yield returns false.
func (s *RawIDs[U]) ValuesBetween(n, m int) []U {
	if s.IsSlice() {
		return s.slice.ValuesBetween(n, m)
	}
	return s.bits.ValuesBetween(n, m)
}
